...
namespace Sgr.DataCategories.Infrastructure.Sead
{
    public class DataCategoryItemAreaDataBaseInitialize : IDataBaseInitialize
    {
        ...
        {
            if (!_context.Set<DataCategoryItem>().Any(f => f.CategoryTypeCode == CategoryTypeProvider.CategoryType_Area_Code))
            {
                //如果未设置
        ...
        private async Task CreateSons(IEnumerable<Area> all, Func<Area, bool> predicate, long pid)
        {
            ...
            foreach (var son in sons)
            {
                ...
                var dataCategoryItem = await _dataCategoryItemManage.CreateNewAsync(son.Name, son.Value, "", CategoryTypeProvider.CategoryType_Area_Code, orderNumber, pid);
