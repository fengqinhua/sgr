...
namespace Sgr.DataCategories.Infrastructure.Sead
{
    public class DataCategoryItemDataBaseInitialize : IDataBaseInitialize
    {
        ...
        {
            await initOnCategoryType(CategoryTypeProvider.CategoryType_SoftType_Code, "category_soft_type.txt");
            await initOnCategoryType(CategoryTypeProvider.CategoryType_SoftClassification_Code, "category_soft_classification.txt");
        }
        ...
        private async Task initOnCategoryType(string categoryTypeCode, string fileName)
        {
            if (!_context.Set<DataCategoryItem>().Any(f => f.CategoryTypeCode == categoryTypeCode))
            {
                ...
                if (File.Exists(filePath))
                {
                    ...
                    {
                        var item = list[i];
                        var dataCategoryItem = await _dataCategoryItemManage.CreateNewAsync(item.Name, item.Value, item.Description, categoryTypeCode, i, 0);
