...
namespace Sgr.BackGroundTasks
{
    public class QuartzGenericJob : IJob
    {
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            {
                //如果无法获取执行任务所需的数据，则退出
                str_data = context.MergedJobDataMap.GetString(QuartzBackGroundTaskManager.Prefix + "TDataObject");
                if (string.IsNullOrEmpty(str_data))
                {
                    _logger.LogError($"任务 QuartzGenericJob 无法从 MergedJobDataMap 中获取任务所需的数据 [ Task Name = {taskMetadata.TaskName} ]，放弃执行 ...");
                    return;
                }
            ...
            using (var scope = _serviceScopeFactory.CreateScope())
            {
                ...
                {
                    _logger.LogError(exceptionOnTask, $"任务 QuartzGenericJob 调用任务的具体实现类失败...");
                    ...
                    if (context.RefireCount > maxRetryCountOnError)
                    {
                        ...
                        _logger.LogError($"任务 QuartzGenericJob 已执行失败超过设定的最大上限 [{maxRetryCountOnError}] ..." +
                            $"-- 参数值：{str_data}");
                        try
                        {
                        ...
                        {
                            _logger.LogError(exceptionOnReport, $"任务 QuartzGenericJob 执行 OnJobNeverSucceed 方法失败...");
                        }
                        return;
                    }
